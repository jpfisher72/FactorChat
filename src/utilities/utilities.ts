export const svgData = (_svg: any): string => {
  ...
  export const meme = (data: Array<any>): string => {
    let memeContent = "";
    data.forEach((motif) => {
      memeContent += `MOTIF ${motif.accession}\n`;
      memeContent += `letter-probability matrix: \n`;
      motif.pwm.forEach((row: number[]) => {
        memeContent += row.join(" ") + "\n";
      });
      memeContent += "\n";
    });
    return memeContent;
